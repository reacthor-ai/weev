import { MessagingRoleValue, reacthorDbClient } from '@/db'
...
export const createBucketStoreTransactionMessaging = async (
  params: CreateBucketStoreTransactionMessagingParams
) => {
  const { bucketStoreData, messagingStoreData, masterBucketId, datastoreId } =
    params
  ...
  try {
    ...
    const [, bucketStore, messagingTable] = await reacthorDbClient.$transaction(
      [updateMasterBucketId, createManyBucketStores, createManyMessagingTable]
    )
...
export type CreateGCPBucketStoreRagParams = Pick<
  CreateBucketStoreTransactionMessagingParams,
  'bucketStoreData'
>

export const createGCPBucketStoreRag = async (
  params: CreateGCPBucketStoreRagParams
) => {
  const { bucketStoreData } = params

  try {
    const createManyBucketStores =
      await reacthorDbClient.gcpBucketStore.createMany({
        data: bucketStoreData
      })
